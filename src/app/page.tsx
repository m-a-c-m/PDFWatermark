...
import Tool from "@/components/PDFWatermark";

const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://miguelacm.es/tools/pdf-watermark";
const EMBED_URL = process.env.NEXT_PUBLIC_EMBED_URL || "https://miguelacm.es/embed/pdf-watermark";

export const metadata: Metadata = {
  title: "PDF Watermark — Free Online Tool",
      ...
      <main className="min-h-screen px-4 py-12">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-center">
            ...
            <h1 className="mb-3 text-4xl font-bold text-white md:text-5xl">PDF Watermark</h1>
            <p className="mb-2 text-lg text-text-muted">Add text watermark to all PDF pages. Configurable position, opacity and color.</p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
            { icon: "💧", title: "Any text", desc: "Stamp a text watermark across every page of the PDF." },
            { icon: "🎨", title: "Full control", desc: "Position, opacity, rotation, size and color are all adjustable." },
            { icon: "🔒", title: "100% private", desc: "Watermarking runs locally with pdf-lib." },
            ].map((item) => (
          ...
          <div className="mt-8 rounded-xl border border-border/20 bg-white/3 p-6">
            ...
            <p className="mb-4 text-sm text-text-muted">Add PDF Watermark to any page with a simple iframe, or link to it with attribution.</p>
            <div className="mb-3 rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Iframe (plug & play):</p>
              <code className="text-xs text-green-400 break-all">{`<iframe src="${EMBED_URL}" width="100%" height="700" style="border:none;border-radius:12px;" title="PDF Watermark — miguelacm.es" loading="lazy"></iframe>`}</code>
            </div>
            <div className="rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Link with attribution (recommended for backlink):</p>
              <code className="text-xs text-green-400 break-all">{`<a href="${SITE_URL}" target="_blank" rel="noopener">PDF Watermark — free tool by MACM</a>`}</code>
            </div>
          </div>
        </div>
      </main>
